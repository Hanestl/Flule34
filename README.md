...
- 详情页内嵌自动播放、元数据、清晰度选择和原生 MP4 播放；
- 播放器保持原比例的沉浸式全屏、前后 10 秒、音量、倍速和紧凑控制栏；
- 按网络类型选择清晰度、播放时屏幕常亮和可配置全屏方向；
- 播放器缓冲反馈、后台自动暂停、进度保存和返回键退出全屏；
- 视频喜欢/不喜欢、分类/标签/艺术家关联投票；
- 发表评论、评论投票、页面评论展示与成功后详情刷新；
...
- 下载入队前刷新视频令牌、401/403/404 自动换令牌重试、系统杀死任务恢复和公共下载目录副本导出；
- 可配置下载并发数与账号下载存储统计；
...
- 播放器、后台下载、公共目录导出和复杂网络切换仍需覆盖 Android 10–16 真机矩阵；
- 成员资料依赖公开页面 `.channel_logo` 结构，页面变化时需更新 Parser fixture；
- 新建播放列表、评论分页和稍后观看写操作仍待可靠接口证据；App 不会猜测参数修改账号数据；
